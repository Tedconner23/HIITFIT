...
- [x] Tier 1 — "fully automated HIIT" batch (2026-07-07) — from `roadmap.md`. (1) **Voice announcements** via `speechSynthesis` in the HIIT player: speaks the exercise on work, "Rest. Next up, X" on rest, warm-up/cool-down/get-ready cues; `cancel()` before each so cues don't lag the timer. (2) **Warm-up / cool-down / rest-between-rounds** added to the HIIT model (optional `warmup`/`cooldown`/`restBetweenRounds` seconds) — `buildTimeline` prepends warm-up, appends cool-down after the trailing-rest trim, and inserts a "Round rest" between rounds (replacing that round's trailing exercise rest so it's not rest-on-rest). New editor fields for all three. (3) **Live progress during play**: progress bar + elapsed / -remaining / "Interval X / N" (prep lead-in excluded from totals). (4) **Settings screen** (`/settings`, gear on the Workouts header): sound / vibration / voice toggles in a new `settings` Pinia store (localStorage, all default on); the player gates `beep`/`vibrate`/`speak` on them. New pure `phaseLabel(kind)` helper. Tests added to `hiit.test.js` (warm-up/cool-down/round-rest/phaseLabel) + new `settings.test.js` — 30 tests, all passing; production build verified. (Node.js LTS 24 was installed via winget this session — the machine had lost/never had it on PATH.) Still needs an in-browser/device pass (voice + new intervals especially).
