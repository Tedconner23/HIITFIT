...
const cues = [
  { key: 'sound', label: 'Sound', hint: 'Beeps on interval changes and the 3-2-1 lead-in.' },
  { key: 'vibration', label: 'Vibration', hint: 'Haptic buzz on each transition (where supported).' },
  { key: 'voice', label: 'Voice', hint: 'Speaks the exercise and next-up during play.' },
]
...
<template>
  <header class="flex items-center justify-between py-6">
    <RouterLink :to="{ name: 'workouts' }" class="text-neutral-400">‹ Back</RouterLink>
    <h1 class="text-lg font-semibold">Settings</h1>
    <span class="w-10"></span>
  </header>

  <p class="mb-4 text-sm text-neutral-400">Cues during HIIT playback.</p>

  <ul class="flex flex-col gap-3">
    <li
      v-for="cue in cues"
      :key="cue.key"
      class="flex items-center justify-between gap-4 rounded-2xl border border-neutral-200 bg-white px-5 py-4"
    >
      <span>
        <span class="block font-medium">{{ cue.label }}</span>
        <span class="mt-0.5 block text-sm text-neutral-400">{{ cue.hint }}</span>
      </span>
      <button
        role="switch"
        :aria-checked="settings[cue.key]"
        :aria-label="cue.label"
        class="relative h-7 w-12 shrink-0 rounded-full transition-colors"
        :class="settings[cue.key] ? 'bg-neutral-900' : 'bg-neutral-300'"
        @click="settings[cue.key] = !settings[cue.key]"
      >
        <span
          class="absolute top-1 h-5 w-5 rounded-full bg-white transition-transform"
          :class="settings[cue.key] ? 'translate-x-6' : 'translate-x-1'"
        ></span>
      </button>
    </li>
  </ul>
